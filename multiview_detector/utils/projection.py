...
def get_worldgrid_from_imagecoord(image_coord, intrinsic_mat, extrinsic_mat):
    ...
    world_grid = np.matmul(imgcoord2worldgrid_mat, image_coord).squeeze()
...
def get_imagecoord_from_worldgrid(worldgrid, intrinsic_mat, extrinsic_mat):
    worldcoord2imgcoord_mat = np.delete(np.matmul(intrinsic_mat, extrinsic_mat), 2, 1)
    worldgrid2worldcoord_mat = np.array([[2.5, 0, -300], [0, 2.5, -900], [0, 0, 1]])
    worldgrid2imgcoord_mat = np.matmul(worldcoord2imgcoord_mat, worldgrid2worldcoord_mat)
    worldgrid = np.append(worldgrid, [1])
    imagecoord = np.matmul(worldgrid2imgcoord_mat, worldgrid[:, np.newaxis]).squeeze()
...
def get_imagecoord_from_worldcoord(world_coord, intrinsic_mat, extrinsic_mat):
    ...
    image_coord = np.matmul(project_mat, world_coord[:, np.newaxis]).squeeze()
